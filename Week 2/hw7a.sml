...
(* CHANGE add shifts for expressions of the form Shift(deltaX, deltaY, exp *)
	 ...
	 | Var of string
	 | Shift of real * real * geom_exp (* represents deltaX, deltaY applied to geom_exp *)
...
fun intersect (v1,v2) =
    case (v1,v2) of
     ...
     | (Point p1, Point p2) => if real_close_point p1 p2
      ...
      | (Point _, LineSegment seg) => intersect(v2,v1) (* same as LineSegment, Point *)

      | (Line _, Point _) => intersect(v2,v1) (* same as Point, Line *)
      ...
      | (Line _, LineSegment _) => intersect(v2,v1) (* handled by case (LineSegment seg, _) *)

      | (VerticalLine _, Point _) => intersect(v2,v1) (* same as Point, VerticalLine *)
      | (VerticalLine _, Line _)  => intersect(v2,v1) (* same as Line, VerticalLine *)
      ...
      | (VerticalLine _, LineSegment seg) => intersect(v2,v1) (* handled by case (LineSegment seg, _) *)
...
fun eval_prog (e,env) =
    ...
    case e of
      ...
      | LineSegment _  => e
      | Var s => 
      	(* search for variable binding in env list *)
		(case List.find (fn (s2,v) => s=s2) env of
		     NONE => raise BadProgram("var not found: " ^ s)
		   | SOME (_,v) => v)
      | Let(s,e1,e2) => 
      	(* eval e2 in environment with value of e1 bound to variable s *)
      ...
      | Shift(deltaX, deltaY, exp) => case eval_prog(exp, env) of
      		...
      		| _ => raise BadProgram("Shift geom_exp evaluated to non-value")
	...
	case e of
		LineSegment seg => let
			...
			in
				...
				else (if x2 < x1 then LineSegment(x2, y2, x1, y1) else e)
			end
		...
		| _ => e (* all other expressions do not need preprocess *)
