...
class GeometryValue 
  ...
  public
  # all GeometryValue knows to intersect self with another GeometryValue
  # which would necessarily be another GeometryValue
  def intersect other 
    raise "MethodMissingError"  # subclasses should implement this method
  end
  ...
  def intersectLineSegment seg
    # double-dispatch in intersect method; selects correct intersect method
    # of the line containing segment based on type of self 
    line_result = intersect(two_points_to_line(seg.x1,seg.y1,seg.x2,seg.y2))
    # handle each case of the intersection result in the method call
...
class Point < GeometryValue
  ...
  private def inbetween(v, end1, end2)
    # helper method to check if point is within segment bounds
...
class Line < GeometryValue
  ...
  def intersectPoint p
    p.intersectLine self  # intersect is commutative
  end
  def intersectLine line
    if real_close(@m, line.m)
      # either same line or parallel lines which do not intersect
      if real_close(@b, line.b) then self else NoPoints.new end
    else
...
class VerticalLine < GeometryValue
  ...
  def intersectPoint p
    p.intersectVerticalLine self  # intersect is commutative
  end
  def intersectLine line
    line.intersectVerticalLine self # intersect is commutative
  end
  def intersectVerticalLine vline
    # either same line or parallel lines which do not intersect
    if real_close(@x, vline.x) then self else NoPoints.new end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(@x1, @y1, @x2, @y2)
      ...
      if @y1 > @y2 then LineSegment.new(@x2, @y2, @x1, @y1) else self end
    else
