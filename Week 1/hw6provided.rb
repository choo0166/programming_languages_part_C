...
class Piece 
  ...
  def initialize (point_array, board)
    ...
    @base_position = [5, 0] # [x offset, y offset]
    @board = board
    @moved = true
  end
...
class Tetris
  ...
  def run_game
    if !@board.game_over? and @running
      # timer event to redraw canvas after delay
      @timer.stop
  ...
  def draw_piece (piece, old=nil)
    ...
    # block coordinates in x,y plane are scaled by block size
    # and translated to form diagonally opposite points of rect
    blocks.map{|block| 
    TetrisRect.new(@canvas, start[0]*size + block[0]*size, 
